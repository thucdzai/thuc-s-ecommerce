...
// Shipping Service vừa CONSUME sự kiện đơn hàng (để biết đơn nào cần giao, giữ bản sao
// orderCode/userId/người nhận phục vụ tạo vận đơn), vừa PUBLISH kết quả giao hàng
// (để Order Service tự động chuyển trạng thái đơn) — cùng kiểu kết hợp producer+consumer
// như Order/Payment Service.
...
async function subscribeAndRun(topic, handler) {
    ...
    await consumer.run({
        eachMessage: async ({ message }) => {
            const envelope = parseEnvelope(topic, message);
            if (!envelope) return;
