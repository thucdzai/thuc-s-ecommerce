...
import { ORDER_STATUS_LABEL, ADMIN_PAGE_SIZE } from '@/lib/constants';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Theo dõi và xử lý đơn hàng toàn hệ thống</p>
      </div>
      ...
      {isError && <ErrorState error={error} message="Không thể tải danh sách đơn hàng" />}
      {!isLoading && !isError && data?.items?.length === 0 && <EmptyState title="Không có đơn hàng nào" />}
            ...
            <TableBody>
              {data.items.map((order) => (
                <TableRow key={order.id}>
                  ...
                  <TableCell>{order.recipient?.name ?? `User #${order.userId}`}</TableCell>
                  <TableCell className="whitespace-nowrap">{formatDateTime(order.createdAt)}</TableCell>
                  <TableCell><OrderStatusBadge status={order.status} /></TableCell>
                  <TableCell><PriceTag value={order.totalAmount} /></TableCell>
